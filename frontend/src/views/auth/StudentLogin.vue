<template>
  <div class="login-container">
    <el-card class="login-card" shadow="hover">
      ...
      <!-- 快速导航 -->
      <div class="quick-nav">
        <el-link type="info" @click="goToTeacherLogin">
          教师登录
        </el-link>
      </div>
    </el-card>
...
const handleLogin = async () => {
  ...
  try {
    ...
    const result = await auth.studentLogin(loginForm)

    if (result.success) {
      ElMessage.success('登录成功')
      const redirect = router.currentRoute.value.query.redirect
      await router.push(typeof redirect === 'string' && redirect ? redirect : { name: 'student-dashboard' })
    } else {
      // 登录失败
